<div>
    <!-- ▸ User-Management Breadcrumb + Toolbar -->
        ...
        <div class="flex items-center justify-between mb-2">
            ...
            <!-- Add User Button moved inside navguide and positioned to the right -->
            <button
                x-data
                @click="$dispatch('open-add-user-modal')"
                {{-- class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 whitespace-nowrap" --}}
    ...
    <div x-data="addUserModal()" 
         x-init="init()"
         @open-add-user-modal.window="openModal"
         @close-add-user-modal.window="closeModal"
         x-cloak
    >
    ...
    x-transition:leave-end="opacity-0"
             class="fixed inset-0 z-50 bg-gray-900/30 dark:bg-gray-900/30 flex  overflow-y-auto items-center justify-center p-4 overflow-hidden"
            @click.away="closeModal"
        >
            <div class="relative w-full max-w-2xl max-h-[calc(100vh-2rem)]" @click.stop>
    ...
    <div x-data="editUserModal()" 
         x-init="init()"
         @open-edit-user-modal.window="openModal"
         @close-edit-user-modal.window="closeModal"
         x-cloak
    >
    <div x-show="showModal"
    x-transition:enter="transition ease-out duration-200"
    ...
    x-transition:leave-start="opacity-100"
    x-transition:leave-end="opacity-0"
             class="fixed inset-0 z-50 bg-gray-900/30 dark:bg-gray-900/30 flex  overflow-y-auto items-center justify-center p-4 overflow-hidden"             @click.away="closeModal"
        >
            <div class="relative w-full max-w-2xl max-h-[calc(100vh-2rem)]" @click.stop>
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <!-- Header -->
    ...
    <script>
        function addUserModal() {
            ...
            // Remove class after a tiny delay to ensure transition starts first
            setTimeout(() => {
